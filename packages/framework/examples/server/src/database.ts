/**
 * Mock Database - In-Memory Data Store (Server Version)
...
class InMemoryDB {
    ...
    private _articles: Article[] = [...mockArticles];
    private _messages: ChatMessage[] = [];

    // Users
    users = {
        findAll: (): User[] => this._users,
        findById: (id: number): User | undefined => this._users.find(u => u.id === id),
        findByEmail: (email: string): User | undefined => this._users.find(u => u.email === email),
    ...
    products = {
        ...
        findById: (id: number): Product | undefined => this._products.find(p => p.id === id),
        ...
        create: (product: Omit<Product, 'id'>): Product => {
            const newProduct = { ...product, id: this._products.length + 1 };
    ...
    articles = {
        ...
        findById: (id: number): Article | undefined => this._articles.find(a => a.id === id),
        findBySlug: (slug: string): Article | undefined => this._articles.find(a => a.slug === slug),
        ...
        create: (article: Omit<Article, 'id'>): Article => {
            const newArticle = { ...article, id: this._articles.length + 1, views: 0 };
    ...
    messages = {
        ...
        create: (message: Omit<ChatMessage, 'id'>): ChatMessage => {
            const newMessage = { ...message, id: this._messages.length + 1 };
            this._messages.push(newMessage);
            return newMessage;
        },
        clear: (roomId: string): void => {
            this._messages = this._messages.filter(m => m.roomId !== roomId);
        },
    };
